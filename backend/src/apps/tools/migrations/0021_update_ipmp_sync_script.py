...
SCRIPT_SUMMARY = "更新 IPMP 项目同步脚本"
SCRIPT_CHANGELOG = "引入 BlueKing CMDB 接口调用与字段映射逻辑，生成资产 JSON。"
...
def update_ipmp_script(apps, schema_editor):
    ...
    repo.content = SCRIPT_CONTENT
    repo.save(update_fields=["content", "updated_at"])

    version, _ = CodeRepositoryVersion.objects.update_or_create(
        repository=repo,
        version=SCRIPT_VERSION,
        defaults={
            "summary": SCRIPT_SUMMARY,
            "change_log": SCRIPT_CHANGELOG,
            "content": SCRIPT_CONTENT,
        },
    )

    repo.latest_version = version
    repo.save(update_fields=["latest_version", "updated_at"])


class Migration(migrations.Migration):

    dependencies = [
        ("tools", "0020_update_zabbix_interface_fields"),
    ]

    operations = [
        migrations.RunPython(update_ipmp_script, migrations.RunPython.noop),
    ]
