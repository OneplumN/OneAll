...
def _transform_host(host: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        # 会写入 AssetRecord.sync_status，用于区分“停用”
        "status": status_label,
        "metadata": metadata,
    }
...
def main(config=None):
    ...
    # 平台会读取 main() 的返回值入库；同时保留 RESULT 兼容。
...
class Migration(migrations.Migration):
    dependencies = [
        ("tools", "0024_update_zabbix_assets_visible_name"),
    ]
