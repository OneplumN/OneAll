...
ZABBIX_USERNAME = CONFIG.get("zabbix_username") or os.getenv("XXX_ZABBIX_USERNAME", "Admin")
ZABBIX_PASSWORD = CONFIG.get("zabbix_password") or os.getenv("XXX_ZABBIX_PASSWORD", "zabbix")
...
def zabbix_request(session: requests.Session, method: str, params: dict, token: str | None = None) -> dict:
    payload = {
        "jsonrpc": "2.0",
        "method": method,
        "params": params,
...
def zabbix_login(session: requests.Session) -> str:
    result = zabbix_request(session, "user.login", {"username": ZABBIX_USERNAME, "password": ZABBIX_PASSWORD})
    print("Zabbix 认证成功")
    return result
...
def main(config=None):
    ...
    global ZABBIX_URL, ZABBIX_USERNAME, ZABBIX_PASSWORD
    ZABBIX_URL = CONFIG.get("zabbix_url") or ZABBIX_URL
    ZABBIX_USERNAME = CONFIG.get("zabbix_username") or ZABBIX_USERNAME
    ZABBIX_PASSWORD = CONFIG.get("zabbix_password") or ZABBIX_PASSWORD
    sync_accounts()
...
def restore_script(apps, schema_editor):
    ...
    version = CodeRepositoryVersion.objects.create(
        repository=repo,
        version="v1.3.0",
        summary="还原账号同步脚本，LDAP 通过环境变量维护",
        change_log="去除多余字段，仅保留 Zabbix 参数",
    ...
    metadata.setdefault("config_fields", [
        {"key": "zabbix_url", "label": "Zabbix API 地址"},
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("tools", "0012_trim_account_sync_metadata"),
    ]

    operations = [
        migrations.RunPython(restore_script, noop),
    ]
