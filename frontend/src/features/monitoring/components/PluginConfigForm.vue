<template>
  <el-card
    shadow="never"
    class="plugin-form integration-config-card"
  >
    <template #header>
      <div class="header">
    ...
    <div v-else>
      ...
      <div
        v-else
        class="plugin-form__body"
      >
        <div class="plugin-form__meta">
          <span
            v-if="plugin?.last_message"
            class="meta-text"
          >{{ plugin.last_message }}</span>
          <span
            v-if="plugin?.last_checked_at"
            class="meta-text"
          >
        ...
        <div v-if="isScriptRuntime">
          ...
          <div class="script-picker">
            <el-input
              ...
              readonly
              :placeholder="defaultScriptLabel ? `默认脚本：${defaultScriptLabel}` : '请选择脚本仓库中的脚本'"
            />
            <el-button
              type="primary"
              plain
...
import { updatePluginConfig, type PluginConfigRecord } from '@/features/monitoring/api/monitoringApi';
