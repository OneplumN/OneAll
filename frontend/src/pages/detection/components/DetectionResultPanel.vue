<template>
  <el-card class="result-panel" shadow="never">
    <template #header>
      <div class="header">
        <span>拨测结果</span>
        <el-tag :type="statusTagType" size="small">{{ detection.status }}</el-tag>
      </div>
    </template>
    ...
    <section v-if="certificate" class="certificate-section">
      <h4>证书信息</h4>
      <el-descriptions :column="1" border>
        <el-descriptions-item label="状态">
          <el-tag :type="certificateTagType" size="small">{{ certificate.status }}</el-tag>
        </el-descriptions-item>
    ...
    <section v-if="detection.error_message" class="error-section">
      <el-alert type="error" :closable="false" show-icon>{{ detection.error_message }}</el-alert>
    </section>

    <section v-if="metadataKeys.length" class="metadata-section">
      <h4>元数据</h4>
      <el-descriptions :column="1" border>
        <el-descriptions-item v-for="key in metadataKeys" :key="key" :label="key">
