  ...
  >
    ...
    <template #content>
      <DetectionResults
        ...
        @clear="clearResult"
      >
        <template v-if="!currentTarget && !submitting" #title-extra>
          输入域名后查询 CMDB 资产信息；本页不发起拨测，仅用于确认资产是否收录与字段详情
        </template>

        <template #actions>
          <el-tag :type="summaryTagType" effect="plain" size="small" class="status-tag">
            {{ summaryStatusText }}
          </el-tag>
          <el-button text size="small" :disabled="!resultRecord" @click="copyRecord">
            复制 JSON
          </el-button>
          <el-button
            text
        ...
        <template #description>
          <el-descriptions :column="2" size="small" class="cmdb-summary">
            <el-descriptions-item label="域名">{{ currentTarget || '-' }}</el-descriptions-item>
            <el-descriptions-item label="最近查询">{{ lastQueriedAtText }}</el-descriptions-item>
            <el-descriptions-item label="状态">{{ summaryStatusText }}</el-descriptions-item>
            <el-descriptions-item label="提示信息">{{ resultMessage || '-' }}</el-descriptions-item>
          </el-descriptions>
        </template>

        <template #columns>
          <el-table-column prop="label" label="字段" min-width="200" />
          <el-table-column prop="value" label="值" min-width="320" show-overflow-tooltip />
        </template>
      </DetectionResults>
    </template>
  </DetectionPageBase>
...
import { validateDomainWithCMDB } from '@/services/detectionApi';
...
async function handleSubmit() {
  ...
  try {
    ...
    submissionError.value = error instanceof Error ? error.message : '查询失败，请稍后再试。';
  } finally {
