<template>
  <component :is="embedded ? 'section' : 'el-card'" v-bind="embedded ? {} : { shadow: 'never' }">
    <template v-if="!embedded" #header>
      <div class="timeline-header">
        <span>审批进程</span>
        <el-tag v-if="!requests.length" type="info">暂无申请</el-tag>
      </div>
    </template>

    <div v-if="embedded && !requests.length" class="timeline-empty">
    ...
    <el-timeline v-else>
      ...
      >
        <div class="timeline-item__title">{{ request.title }} - {{ request.target }}</div>
        <div class="timeline-item__meta">
          协议：{{ request.protocol }}，频率：{{ request.frequency_minutes }} 分钟
        </div>
        <div class="timeline-item__status">
          <el-tag size="small" :type="statusTagType(request.status)">{{ statusText(request.status) }}</el-tag>
          <span v-if="request.status === 'pending'" class="ticket">等待管理员审批</span>
          <span v-else-if="request.status === 'approved'" class="ticket">已生效（可直接编辑，保存后立即生效）</span>
          <span v-else-if="request.status === 'rejected'" class="ticket">已驳回（可修改后重新提交）</span>
        </div>

        <div v-if="request.status === 'rejected' && rejectReason(request)" class="timeline-item__reason">
          驳回原因：{{ rejectReason(request) }}
        </div>

        <div v-if="actionButtonsVisible(request)" class="timeline-item__actions">
          <template v-if="request.status === 'pending' && isAdmin">
            <el-button size="small" type="primary" :loading="busyId === request.id" @click="handleApprove(request)">
              通过
            </el-button>
            <el-button size="small" :loading="busyId === request.id" @click="handleReject(request)">
              驳回
            </el-button>
          </template>

          <template v-else-if="request.status === 'approved' && canEdit(request)">
            <el-button size="small" @click="$emit('edit', request)">编辑</el-button>
          </template>

          <template v-else-if="request.status === 'rejected' && canEdit(request)">
            <el-button size="small" @click="$emit('edit', request)">修改</el-button>
            <el-button size="small" type="primary" :loading="busyId === request.id" @click="handleResubmit(request)">
              重新提交
            </el-button>
          </template>
        </div>
...
import type { MonitoringRequestRecord } from '@/services/monitoringApi';
import { approveMonitoringRequest, rejectMonitoringRequest, resubmitMonitoringRequest } from '@/services/monitoringApi';
