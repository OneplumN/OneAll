...
test.describe('Tools & Knowledge flows', () => {
  ...
  test('knowledge center filters articles and opens drawer', async ({ page }) => {
    test.skip(true, 'Knowledge center UI requires running frontend + backend');
    ...
    await page.fill('input[placeholder="标题 / 内容 / 标签"]', '巡检');
    await page.click('button:has-text("搜索")');
    await page.locator('.article-card').first().click();
    await expect(page.getByRole('dialog').or(page.getByRole('heading', { name: '知识库' }))).toBeTruthy();
  });
});
