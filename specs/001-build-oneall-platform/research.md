...
- **Rationale**: dream.md 指定框架；Vue 3 + Vite 具备插件体系、较小首屏体积与组件化优势，Element Plus 提供企业级组件满足监控、表格、仪表等 UI 需求。
...
- **Rationale**: Django 提供成熟的 ORM、认证与管理后台；Celery 与 Django 配合度高，满足拨测任务、同步作业的异步执行需求。dream.md 已指定 Python + Django。
...
- **Rationale**: dream.md 提到 HTTP 或 gRPC；采用 HTTP 起步兼顾通用性，预留 gRPC 保障扩展；JSON 便于日志与告警对接。
...
- **Rationale**: dream.md 要求自动工单创建与审批回调；REST 接口兼容度高，幂等设计避免重复任务。
